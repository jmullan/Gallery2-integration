...
function g2ic_get_g2_id_title_by_path($path) {
	...
	if (empty($ret)) {
		...
		if (empty($ret)) {
			...
			sprintf($user->lang['G2_LOADENTITIESBYID_FAILED'], $g2itemid) . $user->lang['G2_ERROR'] . $ret->getAsHtml()
			. '</body>' . "\n\n"
			. '</html>';
			exit;
		}
...
function g2ic_get_gallery_dirs_and_files() {
	global $g2ic_rel_path, $g2ic_root_album_path, $g2ic_image_ext_regex, $g2ic_sortby, $user;
	...
	if (!$current_dir = opendir($g2ic_root_album_path . $g2ic_rel_path)) {
		echo '<p>' . $user->lang['g2_directory_error'] . '</p>';
		return false;
	}
...
function g2ic_imageblock($g2itemid) {
	...
	if (empty($ret)) {
		...
		sprintf($user->lang['g2_id_not_found_error'], $g2itemid) . $user->lang['G2_ERROR'] . $ret->getAsHtml()
		. '</body>' . "\n\n"
		. '</html>';
...
function g2ic_imagebypathblock($g2itempath ) {
	...
	if (empty($ret)) {
		...
		echo $user->lang['g2_id_by_path_error'] . $g2itempath . '<br />'
		. '</body>' . "\n\n"
		. '</html>';
		$img = $user->lang['invalid_image'];
	}
...
function g2ic_make_html_about($version) {
	...
	. '\nAdapted and modified for phpBB3<->Gallery2\nby Scott Gregory - jettyrat@jettyfishing.com\')" '
	. 'value="' . $user->lang['about'] . '"/>' . "\n"
	. '</div>' . "\n";
...
function g2ic_make_html_dir_menu(){
	global $g2ic_rel_path, $g2ic_dirs_titles, $g2ic_sortby, $g2ic_display_filenames, $g2ic_images_per_page, $user;

	$html = '<div>' . "\n"
	. '    <fieldset>' . "\n"
	. '        <legend>' . $user->lang['current_album'] . '</legend>' . "\n"
	...
	if (!empty($g2ic_dirs_titles)) {
		$html .= '    <fieldset>' . "\n"
		. '        <legend>' . $user->lang['subalbums'] . '</legend>' . "\n"
		...
		$html .=	'            </select>' . "\n"
		. '            <input type="button" name="Submit" value="' . $user->lang['go'] . '" onClick="top.location.href = this.form.subalbums.options[this.form.subalbums.selectedIndex].value; return false;">' . "\n"
		. '        </form>' . "\n"
		. '    </fieldset>' . "\n";
	}
...
function g2ic_make_html_display_options(){
	global $g2ic_sortby, $g2ic_rel_path, $g2ic_images_per_page, $g2ic_display_filenames, $user;
	...
	// array for output
	$sortoptions = array(
		'title_asc' => array('text' => $user->lang['title_a_to_z']),
		'title_desc' => array('text' => $user->lang['title_z_to_a']),
		'name_asc' => array('text' => $user->lang['name_a_to_z']),
		'name_desc' => array('text' => $user->lang['name_z_to_a']),
		'mtime_desc' => array('text' => $user->lang['last_modification_new']),
		'mtime_asc' => array('text' => $user->lang['last_modification_old'])
	);
	...
	$html .=	'            </select>' . "\n"
	. '            <input type="submit" value="' . $user->lang['redraw'] . '" /><br />' . "\n"
	. '            <input type="radio" name="display" value="thumbnails"';
	$html .= (empty($g2ic_display_filenames)) ? ' checked="checked"' : '';
	$html .= ' onclick="showThumbnails()">' . $user->lang['thumbnails'] . '</input>' . "\n";

	$html .= '            <input type="radio" name="display" value="filenames"';
	$html .= (!empty($g2ic_display_filenames)) ? ' checked="checked"' : '';
	$html .= ' onclick="showFileNames()">' . $user->lang['filenames'] . '</input>' . "\n";
...
function g2ic_make_html_image_navigation() {
	global $g2ic_image_files, $g2ic_current_page, $g2ic_images_per_page, $g2ic_rel_path, $g2ic_sortby, $g2ic_g2thumbnail_src, $g2ic_thumbnail_width, $g2ic_thumbnail_height, $g2ic_thumbnail_alt_text, $g2ic_image_url, $g2ic_album_url, $g2ic_custom_url, $g2ic_display_filenames, $g2ic_class_mode, $g2ic_form, $g2ic_field, $g2ic_host, $user;
	...
	foreach ($g2ic_image_files as $key => $row) {
		...
		if ($key >= $g2ic_current_page * $g2ic_images_per_page) {
			...
			$html .= '        ' . $user->lang['image_title'] . htmlspecialchars($image_title) . '<br />' . "\n";

			$html .= '        ' . $user->lang['filename'] . htmlspecialchars($image_filename) . "\n";
			...
			. '            <input type="hidden" name="g2ic_host" value="' . $g2ic_host . '" />' . "\n"
			. '        </fieldset>' . "\n\n"
			. '    </form>' . "\n"
			. '</div>' . "\n\n";
		}
...
function g2ic_make_html_page_navigation() {
	global $g2ic_image_files, $g2ic_display_filenames, $g2ic_images_per_page, $g2ic_current_page, $g2ic_rel_path, $g2ic_sortby, $user;

	//Check if the current directory is empty - print empty message and return if empty
	if (empty($g2ic_image_files)) {
		$html = '<p><strong>' . $user->lang['empty_album'] . '</strong></p>' . "\n\n";
	...
	if (count($pagelinks) > 1) {
		$html = '<div>' . "\n"
		. '    <fieldset>' . "\n"
		. '        <legend>' . $user->lang['page_navigation'] . '</legend>' . "\n"
		. '        ' . $user->lang['page'] . ' ' . "\n"
		. implode("     - \n", $pagelinks)
		. "\n"
		. '    </fieldset>' . "\n"
